...
TEST_F(ParticleLayoutTest, ParticleLayoutAddParticle)
{
    ...
    EXPECT_TRUE(NULL == pInfo1->getParticle()->createTransform3D());

    const ParticleInfo * pInfo2 = particleDecoration.getParticleInfo(size_t(1));
    EXPECT_TRUE(NULL!=pInfo2);
    EXPECT_EQ(2.1, pInfo2->getDepth());
    EXPECT_EQ(2.2, pInfo2->getAbundance());
    EXPECT_TRUE(NULL == pInfo2->getParticle()->createTransform3D());
    ...
    EXPECT_TRUE(NULL != pInfo3->getParticle()->createTransform3D());
    ...
    EXPECT_TRUE(NULL != pInfo4->getParticle()->createTransform3D());
}
