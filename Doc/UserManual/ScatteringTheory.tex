...
We rewrite the scalar Helmholtz equation~\refeq{Helmholtz} as
...
\end{equation}
Eq.~\refeq{Helmholtz} looks like an inhomogenous differential equation ---
...
\end{equation}
The full inhomegenous equation~\refeq{HelmholtzInhomog} is hence
formally solved by
...
by the full right-hand side of~\refeq{PsiFormal}
we obtain
\begin{equation}
  \Psi(\vect{r})
  = \Psi_0(\vect{r})
...
so that the integral in~\refeq{Born} runs over $\vect{r'}$ with $r'\ll r$.
...
\end{equation}
We apply this to the Green's function~\refeq{Greens1},
