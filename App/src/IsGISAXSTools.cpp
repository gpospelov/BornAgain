...
TLine *IsGISAXSTools::getOutputDataScanLine(const OutputData<double>& data)
{
    ...
    if( data.getAxis(BA::ALPHA_AXIS_NAME) &&
            data.getAxis(BA::ALPHA_AXIS_NAME)->getSize() == 1) {
        // horizontal line
    ...
    }else if( data.getAxis(BA::PHI_AXIS_NAME) &&
            data.getAxis(BA::PHI_AXIS_NAME)->getSize() == 1 ) {
        // it's vertical line
        ...
        const OutputData<double>& data, const std::string& hname)
{
    ...
    if( data.getAxis(BA::ALPHA_AXIS_NAME) &&
        ...
        ostr_title << hname << ", alpha_f=" <<
                data.getAxis(BA::ALPHA_AXIS_NAME)->getMin();
    }else if( data.getAxis(BA::PHI_AXIS_NAME) &&
              data.getAxis(BA::PHI_AXIS_NAME)->getSize() == 1 ) {
        hist1 = hist2->ProjectionY();
        ostr_title << hname << ", phi_f=" <<
                data.getAxis(BA::PHI_AXIS_NAME)->getMin();
    } else {
        throw LogicErrorException("IsGISAXSTools::getOutputDataScanHist()"
