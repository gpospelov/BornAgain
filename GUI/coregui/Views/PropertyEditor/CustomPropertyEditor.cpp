...
//
//! @file      coregui/Views/PropertyEditor/CustomPropertyEditor.cpp
//! @brief     Implements class CustomPropertyEditor
//!
...
CustomPropertyEditor::CustomPropertyEditor(QItemSelectionModel *selection_model,
    ...
    , m_browser_type(browser_type)
{
    ...
    if(m_browser_type == BROWSER_TREE_TYPE) {
        ...
        throw GUIHelpers::Error("CustomPropertyEditor::CustomPropertyEditor() -> Error. Unknown browser type.");
    }
...
void CustomPropertyEditor::setSelectionModel(QItemSelectionModel *selection_model)
{
...
void CustomPropertyEditor::selectionChanged(const QItemSelection & selected,
...
void CustomPropertyEditor::slotValueChanged(QtProperty *property,
                                            const QVariant &value)
{
    qDebug() << "CustomPropertyEditor::slotValueChanged()" << value;
    ...
    if (item_index_pair.m_item) {
        ...
        qDebug() << "CustomPropertyEditor::slotValueChanged() -> setting ..." << prop_list[item_index_pair.m_index].constData();
...
void CustomPropertyEditor::clearEditor()
{
    qDebug() << "CustomPropertyEditor::clearEditor()";
...
void CustomPropertyEditor::updateSubItems(const QString &name)
{
    qDebug() << "CustomPropertyEditor::updateSubItems()";
    Q_UNUSED(name)
...
//    disconnect(m_manager, SIGNAL(valueChanged(QtProperty *, const QVariant &)),
//                this, SLOT(slotValueChanged(QtProperty *, const QVariant &)));
...
void CustomPropertyEditor::onPropertyChanged(const QString &property_name)
{
    qDebug() << "CustomPropertyEditor::onPropertyChanged() " << property_name ;
    if(!m_item) return;
...
void CustomPropertyEditor::onPropertyItemPropertyChanged(const QString &property_group, const QString &property_name)
{
    qDebug() << "CustomPropertyEditor::onPropertyItemPropertyChanged" << property_group << property_name;
...
void CustomPropertyEditor::setItem(ParameterizedItem *item)
{
    qDebug() << "CustomPropertyEditor::setItem(ParameterizedItem *item)" << item;
...
void CustomPropertyEditor::setCreateGroupProperty(bool create_group_property)
{
    m_create_group_property = create_group_property;
}


void CustomPropertyEditor::addItemProperties(const ParameterizedItem *item)
{
...
void CustomPropertyEditor::addSubProperties(QtProperty *item_property,
