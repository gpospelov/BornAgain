...
\begin{equation}
  \chi_\text{m}(\q)
  = \chi_\text{m} \int_{z_\il}^{z_{\il-1}}\!\d z \int\!\d^2r_\parallel\, \e^{i\v{q}\,\r}
...
We then collect several terms into the \textit{DWBA form factor}
\begin{equation}
...
\begin{equation}
  \frac{\partial\sigma}{\partial\Omega}
  = \sum_{lm} \sum_{ij} \e^{i(\q_{l\parallel}\R_i-\q_{m\parallel}\R_j)}
