...
\begin{equation}
  ...
  &=& \displaystyle
      ...
      \int_0^{H_0\eta}\!\d z\,\e^{i(q_z-\q_\parallel \V_j/H_0)z}
\\[3ex]
  &=& \displaystyle
       q_\parallel^{-2} H_0\, \sum_j b_j  
          \frac{\e^{i\left(q_zH_0-\q_\parallel\V_j\right)\eta}-1}
               {i\left(q_z H_0 - \q_\parallel\V_j\right)}
               \e^{i\q_\parallel \V_j}.
  \end{array}
\end{equation}
Delegating the handling of the removable singularity at~$q_zH_0=\q_\parallel\V_j$
to the sinc function,
we write it as
\Emph{
\begin{equation}
  F(\q)
  = q_\parallel^{-2} H_0\, \e^{iq_z H_0/2} \sum_{j=0}^{N-1} b_j
...
\begin{equation}
  F(\q)
  = \frac{2i}{q_\parallel}H_0(1-\frac{\eta}{2})\, \e^{iq_z H_0/2}
