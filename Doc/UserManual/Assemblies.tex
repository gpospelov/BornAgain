...
over one layer
so that $\nz(z)=:\overline{n_j^2}$ becomes $z$ independent.

\Warn{\indent Currently, \BornAgain\ only supports
  the dilute-particle limit. Whatever the actual particle concentration,
  the approximation $x_\text{p}(z)=0$ is used so that $\overline{n_j^2}\equiv n_j^2$.}
...
We now definitively specialize to the case of discrete layers~$j$
with constant~$\nz(z)=n_j^2$.
Hence the $\psi_{w}$ consist of plane waves,
and $\bra\psi_\tf|\chi|\psi_\ti\ket$ is given by (\ref{EtmDWBAft}).
We only need to compute the restricted Fourier transform
$\chi_j(\q)$ introduced in (\ref{Echij}).
According to (\ref{Echi0p}),
it has two contributions,
\begin{equation}\label{Echijmp}
  \chi_j(\q) = \chi_\text{m}(\q) + \chi_\text{p}(\q).
\end{equation}
The first one is the shape transform of the entire layer,
\begin{equation}
  \chi_\text{m}(\q)
  = \chi_\text{m} \int_{z_j}^{z_{j-1}}\!\d z \int\!\d^2r_\parallel\, \e^{i\v{q}\,\r}
  = \chi_\text{m} \delta(\q_\parallel) \sinc\left( q_\perp z \right).
\end{equation}
...
For the second term in (\ref{Echijmp}),
we introduce the potential $\chi_\alpha(\r)$
of a single particle $\alpha$ located at the origin,
and the particle location $\R_\alpha$,
to write
\begin{equation}
  \chi_\text{p}(\r) = \sum_\alpha \chi_\alpha(\r-\R_\alpha).
\end{equation}
We further introduce the \textit{form factor}
\index{Form factor}%
of the particle,
\begin{equation}
  F_\alpha(\q) := \int\!\d^3r\,\e^{i\q\r}\chi_\alpha(\r).
\end{equation}
We obtain
\begin{equation}
  \chi_\text{p}(\q) = \sum_\alpha \e^{i\q\R_\alpha} F_\alpha(\q).
\end{equation}
Implemented single-particle form factors 
are listed in great detail in chapter~\ref{sec:FormFactors}.
The positional phase factors $\e^{i\q\R_\alpha}$
...
\begin{align*}
  ...
  & \times \iint_S d^2r_\alpha^\parallel d^2r_\beta^\parallel \ppcf{\alpha}{\beta}{R^\parallel} \exp \left[ i\v{q}_{j\parallel}\cdot (\r_\alpha^\parallel - \r_\beta^\parallel ) \right].
\end{align*}
