...
has a volume~$V_i$ and
a perturbative potential~$\chi_i(\r-\r_i)$.
The embedding material has a constant potential~$\chi_0$.
The average potential of the entire layer is then
\begin{equation}\label{Emulaynav}
  \begin{split}
  \overline{\chi}
      &= \frac{\sum_i \int_{V_i}\!\d^3r\; \chi_i(\r-\r_i)
               + (Ad-\sum_i V_i)\chi_0}
             {Ad}\\[1em]
      &= \chi_0 + \frac{\sum_i \int_{V_i}\!\d^3r\, [\chi_i(\r-\r_i)-\chi_0]}
             {Ad}
  \end{split}
\end{equation}
