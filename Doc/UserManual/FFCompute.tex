...
It seems that \textE{all} analytical form factors have a singularity at~$\q=0$.
Furthermore, if the particle has \textE{plane} surfaces,
...
\end{equation}
If the \textE{sinus cardinalis} is written as
...
we obtain
\Emphc{
\begin{equation}\label{Eff3d3}
  F(\q,\Pi) = 2  \sum_k^{\text{faces of~$\Pi$}} \uq^*\n_k \sum_j^{\text{edges of $\Gamma_k$}}
           (\n_k\times\uq_{\parallel k}^*)\,\E_j\;
           \Delta(q,\frac{q_{\parallel k}}{q},
                 \uq_{\parallel k}\E_j,\uq_{\parallel k}\R_j,\frac{q_{\perp k}}{q}\r_{\perp k})
\end{equation}
with
\begin{equation}
  \Delta(q,v,a,b,c)
  \coloneqq
  \frac{\e^{iqc}\Phi(qv,a,b)-\Phi(0,a,b)} {iq}
\end{equation}
and
\begin{equation}
  \Phi(p,a,b)
  \coloneqq
  \frac{\sinc(pa)\e^{ipb}-1}{ip}.
\end{equation}
}
