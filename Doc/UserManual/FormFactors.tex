...
\chapter{Form factor computations}  \label{SFFcomp}

\BornAgain\ supports embedded nanoparticles of various shapes.
A catalogue of implemented shapes is provided in Appendix~\ref{SFF}.
Here we give some background on how the particle form factor is computed.

The form factor~$F(\q)$ of a hard-shell particle depends
only on the particle's shape~$S(\r)$, and therefore is also called
the \E{shape transform}:
\nomenclature[2t020]{$S(\r)$}{Shape of nanoparticle}%
