...
and the Bessel function of first kind and first order $J_1(z)$ \cite{AbSt64}.
...
Notations:
\begin{equation*}
  R_z \coloneqq R-\dfrac{z}{\tan \alpha}, \quad
  q_{\parallel} \coloneqq \sqrt{q_x^2+ q_y^2}
\end{equation*}
Results:
\begin{equation*}
F = \int_0^H \!\d z\, 2\pi R_z^2
        \frac{J_1(q_{\parallel}R_z)}{q_{\parallel} R_z}\exp(iq_z z),
\end{equation*}
\begin{equation*}
  V = \dfrac{\pi}{3} R^3\tan\alpha  \left[ 
...
\paragraph{Form factor etc}\strut\\\strut\\
...
\begin{align*}
F = \frac{4\sqrt{3}}{3q_y ^2 - q_x^2}
    \int_0^H\!\d z &\exp(iq_z z)
\Big[\frac{3}{4}R_z^2q_y^2 \sinc\left(\frac{q_xR_z}{2}\right)\sinc\left(\frac{\sqrt{3}q_y
R_z}{2}\right)\\
&+\cos(q_xR_z)-\cos\left(\frac{\sqrt{3}q_y R_z}{2}\right)\cos\left(\frac{q_xR_z}{2}\right) \Big]
\end{align*}
with $R_z=R-\dfrac{2z}{\sqrt{3}\tan\alpha}$.

\begin{equation*}
  V = \dfrac{3}{4} R^3 \tan\alpha  \left[
