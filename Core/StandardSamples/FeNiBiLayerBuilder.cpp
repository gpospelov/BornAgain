...
std::unique_ptr<MultiLayer> FeNiBiLayer::constructSample()
{
    ...
    auto m_Fe = effectiveSLD == 0
            ? MaterialBySLD("Fe", sldFe.real(), sldFe.imag(), magnetizationVector)
            : MaterialBySLD("Fe", sldFe.real() + effectiveSLD * rhoMconst *
                            magnetizationMagnitude, sldFe.imag(), kvector_t{0, 0, 0});
...
{
    auto sample = FeNiBiLayer{FeNiBiLayer::Options().sigmaRoughness(2.*Units::angstrom).
