...
\begin{equation}\label{Edwba_ml0}
  \bra \psi_\si|\delta v|\psi_\sf\ket
  = \sum_{\il} \sum_{\pm_\si} \sum_{\pm_\sf}
    A^{\pm *}_{\si \il} A^\pm_{\sf \il}
     \delta v_\il(\k^\pm_{\sf \il}-\k^\pm_{\si \il})
\end{equation}
...
Since $\k_\plll=\text{const}$,
 all four wavevectors $\q^u$ have the same horizontal component~$\q_\plll$;
