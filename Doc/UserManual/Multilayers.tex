...
\begin{equation}
  M_\il
   \coloneqq \frac{1}{2f_{\il}}
   \left(\begin{array}{cc}
       \delta_\il^*(f_{\il}+f_{\il+1})&\delta_\il^*(f_{\il}-f_{\il+1})\\
       \delta_\il  (f_{\il}-f_{\il+1})&\delta_\il  (f_{\il}+f_{\il+1})
   \end{array}\right).
\end{equation}
Wave propagation across the entire stack of layers
is described by the compound transfer matrix
\begin{equation}
   M \coloneqq M_0 \cdots M_{N-1}.
\end{equation}
...
\begin{equation}
  ...
  = M \left( \begin{array}{c}A^-_N\\0\end{array} \right),
\end{equation}
which is solved by
\begin{equation}
  \begin{array}{lcl}
  ...
  \end{array}
\end{equation}
To make a connection with a well known textbook result,
consider a system
