...
but by its \E{vacuum wavenumber}
...
In the following, we will use~\cref{ESchrodi2}
...
It is customary to express $\mv(\r)$ by the neutron optical \E{refractive index}
...
can be derived \cite{DiWa84} along the lines of~\Cref{SfarDW}
except that differential operators and Green functions are now dyadic tensors.
\index{Green function!distorted waves, far-field|(}%
In place of \cref{EDDdef}, we will need the tensorial operators
\begin{equation}
  \begin{array}{@{}l}
    \TDo \coloneqq \ONE(\Nabla^2+K^2)-\Nabla\cdot\Nabla, \\[1.4ex]
    \TD  \coloneqq \TDo + \ONE\delta k(\r)^2,
  \end{array}
\end{equation}
with $\delta k^2\coloneqq k^2-K^2$ as before.
Based on~\cref{EwaveE3},
we posit homogeneous differential equations for plane waves $\Eo_\sif$ and
distorted waves $\v{E}_\sif$,
and inhomogeneous equations that determine the corresponding Green functions:
\begin{equation}
  \begin{array}{l@{\hspace{2em}}l}
    \TDo\Eo_\sif  (\r) = 0, &\TDo\TGo(\r,\r') = \ONE\delta(\r-\r'), \\[1.4ex]
    \TD\v{E}_\sif(\r) = 0, &\TD  \TG  (\r,\r') = \ONE\delta(\r-\r').
  \end{array}
\end{equation}
In analogy with \cref{EGoFF,Epfo},
...
\end{equation}
as can be verified by operating on both sides with~$\TDo$.

We only consider vacuum waves with unit amplitudes $\he_\sif$,
\begin{equation}\label{EXee}
  \Eo_\si(\r) = \he_\si\,\e^{i\k_\si\r}
\text{\strut~and~\strut}
  \Efoc(\r) = \he_\sf\,\e^{-i\k_\sf\r}
\end{equation}
The amplitudes must be perpendicular to the propagation directions, $\he\k=0$.
...
We multiply both sides of~\cref{EXGoFF} from the left with $\hef$,
and both sides of~\cref{EXpfo} with~$\phi(\rD)$ to obtain
\begin{equation}\label{EXGoFF2}
  \hef\phi(\rD)\e^{-i\k_\sf\rS}
  = \hef\TG_\infty(\rD,\rS) + \int\!\d^3r\, \hef\TG_\infty(\rD,\r)\delta k(\r)^2 \TGo(\r,\rS)
\end{equation}
and
...
\begin{equation}\label{EGpE}
  \hef\TG_\infty(\rD,\rS) = \phi(\rD)\Efc(\rS).
\end{equation}
This is analogous to~\cref{Epfo},
and means that we can determine the scattered far field at the detector position~$\rD$
by backtracking the detected plane wave $\Efc$ into the sample,
where it differs from the vacuum wave~$\Efoc$.
...
\begin{equation}
  ...
  = \phi(r) \int\!\d^3r'\, \Efc(\r')\delta v(\r')\v{E}_\si(\r').
\end{equation}
The transcription of \Cref{SdiffCross} from quantum-mechanical probability flux
to the electrodynamical Poynting vector provides no new insights,
and shall be skipped.
The X-ray scattering cross section for given incoming and detected polarization is
\begin{equation}\label{Exsection}
  \xElas
