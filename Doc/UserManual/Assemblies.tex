...
{This generalizes the definition in \IsGISAXS\ \cite[Sect.~2.4.1]{Laz08}
...
\footnotetext[\value{footnote}]{Known from \IsGISAXS\ \cite[Sect.~3.3]{Laz08}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Particle distribution models}\label{Spartidis}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{equation}\label{EPDdeltas}
  ...
  = \delta(\r-\R_{i\plll})\delta(\r'-\R_{j\plll})
...
we assume that the particle distribution allows
the following approximations:
\begin{itemize}[leftmargin=2\parindent,rightmargin=\parindent]
\item[\textbf{(PD1)}] On the length scales considered here
(below the coherence length),
the sample structure is translation invariant.
\item[\textbf{(PD2)}] All particle properties
(like shape, size, and orientation, but excluding position)
are independent and identically distributed (\E{i.i.d.})
random variables.
\end{itemize}
Unless otherwise said, we further assume:
\begin{itemize}[leftmargin=2\parindent,rightmargin=\parindent]
\item[\textbf{(PD3)}] Particle positions do not depend on particle properties.
\end{itemize}}
Altogether, PD1--3 make up the \E{decoupling approximation} 
\index{IsGISAXS@\IsGISAXS!decoupling approximation}
of \IsGISAXS.\footnote
{In the \IsGISAXS\ manual \cite[Sect.~2.2]{Laz08},
...
The LMA
is an alternative to PD1.
It describes the incoherent average of
scattering from different coherence domains.
In \BornAgain,
it can be implemented by averaging over different simulated GISAS images
(as described in Sect.~\ref{Scoherlen}),
or by attaching different \texttt{ParticleLayout}s to a \texttt{Layer}.
The SSCA is an alternative to PD3.
In \IsGISAXS, and similarly in \BornAgain,
it is only available for paracrystals,
and will therefore be discussed later (Sect.~\ref{}).}

Translation invariance (PD1)
allows us to replace $\sum_{ij}\PD_{ij}$ by a distribution function
$\PD(\r-\r',\v\tau,\v\tau')$,
and to get rid of one of the two integrals over particle positions.
After substituting $\r-\r'\to\r$,
the cross section (\ref{Epdm1}) takes the form
\begin{equation}\label{Epdm1}
  \left.\frac{\partial\sigma}{\partial\Omega}\right|_l
  = 
