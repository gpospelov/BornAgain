        ...
        const Bin1DCVector& k_f_bin, double alpha_i, double alpha_f) const
{
    ...
    m_term_S = ai_RT.second*mp_form_factor->evaluate(k_i, k_f_bin,
            alpha_i, alpha_f)*af_RT.second;
    m_term_RS = ai_RT.first*mp_form_factor->evaluate(k_itilde, k_f_bin,
            alpha_i, alpha_f)*af_RT.second;
    m_term_SR = ai_RT.second*mp_form_factor->evaluate(k_i, k_f_bin_tilde,
            alpha_i, alpha_f)*af_RT.first;
    m_term_RSR = ai_RT.first*mp_form_factor->evaluate(k_itilde, k_f_bin_tilde,
            alpha_i, alpha_f)*af_RT.first;
}
