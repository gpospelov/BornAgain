...
//        if( (*it)->getName() == name ) throw LogicErrorException("FitSuiteParameters:addtFitParameter() -> Error. Existing parameter '"+name+"'");
//    }
//    push_back(new FitParameterLinked(name, value, step, attlim, error) );
//}
...
//    if( !valuesAreDifferrent(pars_values) ) {
//        std::cout << "FitSuiteParameters::setValues() -> Warning! Small or absent change in parameter values." << std::endl;
//        for(size_t i=0; i<size(); i++)  std::cout << ((*this)[i]->getValue() -pars_values[i]) << " " << Numeric::areAlmostEqual( (*this)[i]->getValue(), pars_values[i]) << std::endl;
//    }
//    size_t index(0);
//    for(iterator it=begin(); it!=end(); ++it) (*it)->setValue(pars_values[index++]);
//}
...
//void FitSuiteParameters::setValues(const std::vector<double> &pars_values)
//{
//    if(pars_values.size() != size() ) throw OutOfBoundsException("FitSuiteParameters::setValues() -> Wrong size of array with parameter values");
//    setValues(&pars_values[0]);
//}
