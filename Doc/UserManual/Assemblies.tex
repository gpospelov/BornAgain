...
\begin{equation}\label{EPDdeltas}
  \PD_{l,ij}(\r_i,\r_j,\v\tau_i,\v\tau_j)
  = \delta^{(2)}(\r_i-\R_{i\plll})\delta^{(2)}(\r_j-\R_{j\plll})
  \delta^{(d_T)}(\v\tau_i-\v{T}_i)\delta^{(d_T)}(\v\tau_j-\v{T}_j)
\end{equation}
...
The differential cross section~\ref{Eass3} can now
be rewritten as
\begin{equation}\label{Epdm1}
  \left.\frac{\partial\sigma}{\partial\Omega}\right|_l
  = \sum_{ij}
  \int\!\d^2r_i d^2r_j \d^{d_T}\tau_i \d^{d_T}\tau_j
    \PD_{l,ij}(\r_i,\r_j,\v\tau_i,\v\tau_j)
  \e^{i\q_{l\plll}(\r_i-\r_j)}
    \FD^*_l(\v\tau_i) \FD_l(\v\tau_j).
\end{equation}
The integration over $\r$ is restricted to the horizontal plane.
Since all of the following models are concerned with only one layer,
we will from now on
omit the index~$l$ from $\q_{l\plll}$, $\PD_{l,ij}$, and $\FD_l$.
