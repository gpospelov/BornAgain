...
\index{Scattering!target}%
\index{Target}%
typically is a material sample one wants to investigate.
\index{Material!sample}%
\index{Sample}%
It has finite size, and is
surrounded by an infinite homogeneous medium,
typically vacuum or air.
\index{Air}%
\index{Vacuum}%
\index{Surrounding medium}%
\index{Medium}
We write $\overline{v}$ for the SLD of the surrounding medium, and
\begin{equation}\label{Edecompose_v}
  \delta v(\r) \coloneqq v(\r) - \overline{v}
\end{equation}
...
for the modulation of the SLD by the sample.
...
For the special case of grazing incidence,
the distorted wave Born approximation (DWBA, \Cref{SDWBA})
...
will be reassigned from $\delta v(\r)$ to $\overline{v}(z)$
...
\index{Incident radiation|see{Exciting wave}}%
...
For the Born approximation to be accurate,
the perturbation~$\delta v(\r)$ must be small.
In the standard theory, as exposed in~\cref{?}

When this is not the case,
the basic idea of the Born expansion can still be saved
in what is called
...
\begin{equation}\label{ESchrodiKr}
  \left\{ \Nabla^2 + K^2(\r) \right\}\psi(\r)
  = 4\pi\delta v(\r)\psi(\r).
\end{equation}
As before,
the Born expansion starts from the homogeneous wave equation,
\begin{equation}\label{EHomoK}
  \left\{ \Nabla^2 + K^2(\r) \right\}\psi(\r) = 0.
\end{equation}
Differently from \Cref{SBornExpans},
solutions are no longer plane waves.
This makes it necessary
to work out the distinction between \E{exciting} wave~$\psi_\se$
\nomenclature[1ψ034 2e000]{$\psi_\se(\r)$}{Exciting wave}%
and \E{incident} wave~$\psi_\si$.
The \E{exciting wave}
\index{Exciting wave}%
\index{Wave!exciting}%
is prepared far
...
Besides $\psi_\si$,
we will need another solution $\psi_\sf$ of~\cref{EHomoK}
\index{Final wave!DWBA}%
\index{Wave!final}%
to describe a wave field that emanates from the sample,
and reaches a detector pixel in form of a plane wave.
The actual solution of~\cref{EHomoK} shall be postponed to \Cref{sec:Multilayers}.
From here onwards, we proceed as if $\psi_\si(\r)$ and $\psi_\sf(\r)$ were known.
