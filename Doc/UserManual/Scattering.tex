...
To derive the scattering cross section in DWBA,
we are going to use generic properties of the Green function~$G$
\index{Green function!DWBA}%
that do not depend on the specific form~\cref{EHelmholtzHomog}
of the homogeneous wave equation.
While ignoring the full solution~$G(\r,\r')$,
we can still investigate its asymptotic far-field limit $G(\rD,\r')$
...
its replacement depends on the distorted wavefunction,
is therefore application specific,
and will be worked out later (\cref{SdwbaMulay}).
...
\index{Backtracing!beam footprint}%
