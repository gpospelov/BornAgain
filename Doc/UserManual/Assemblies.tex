...
BornAgain only supports step profiles for~$v_\sm(z)$.
Within one layer~$l$, the matrix has a constant SLD~$v_{\sm l}$.
To specifiy a decomposition~\cref{Edecompose_vz}, we define
\begin{equation}
  \delta v_l(\r)
  \coloneqq\left\{\begin{array}{ll}
  v_l(\r) - \overline{v}_l&\text{~if $\r$ in Layer~$l$,}\\[.2ex]
  0&\text{~otherwise.} \end{array}\right.
\end{equation}
This leaves the constant term $\overline{v}_l$ still undetermined.
The best choice is probably the volume average of $v(\r)$,
though we are not aware of any proof that this maximizes the accuracy of the DWBA.
\Note{Currently (at least up to version 1.7),
BornAgain uses the simpler choice
 $\overline{v}_l = v_{\sm l}$.
This may change in the future.}
Per \Cref{SStep}, $\overline{v}_l$ determines the refractive index~$n_l$
and the wavenumber~$K_l$.
The wavefunctions $\psi_\si$ and~$\psi_\sf$
are then determined as described in \Cref{Sacrolay}.
Furthermore, our current choice $\overline{v}_l = v_{\sm l}$
implies
\begin{equation}\label{Edvl_cur}
  \delta v_l(\r) = \Delta v_\sp(\r)\text{~for $\r$ in the layer.}
\end{equation}
\Warn{Currently,
BornAgain does not compute $\Delta v_\sp = v_\sp - v_\sm$,
but interprets the given $v_\sp$
(given through a particle refractive index) as if it were $\Delta v_\sp$.}
...
which thanks to~\cref{Edvl_cur} takes the simple form
\Emph{
\begin{equation}\label{Edvlq}
  \delta v_l(\q)
  = \int\!\d^3r\, \e^{i\v{q}\,\r}\delta v_l(\r)
\end{equation}
\vspace*{-5pt}}
...
we can work with
\Emph{
\begin{equation}\label{Edvlq2}
  \delta v_l(\q)
  = \Delta v_\sp\chi_\sp(\q).
\end{equation}
\vspace*{-5pt}}
\fi      %----------------------------------------------------------------------
...
\begin{equation}\label{ESqFq}
  v_\sp(\q) = S(\q) F(\q)
\end{equation}
of the assembly's \E{structure factor}, given by the particle positions,
\begin{equation}\label{EdefSq}
  S(\q) \coloneqq \sum_i \e^{i\q\R_i},
\end{equation}
and the individual particles' \E{form factor}
\begin{equation}\label{EdefFq}
  F(\q) \coloneqq \int\!\d^3r\, \e^{i\v{q}\,\r} F(\r).
\end{equation}
Form factors are discussed in \Cref{SFormFactor}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Form factors}\label{SFormFactor}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
