...
or dielectric permittivities, namely the \E{refractive index},
which is defined if the distortion field~$\Lambda(\r)$ is scalar:
\begin{equation}\label{EnkK}
  n(\r)\coloneqq\sqrt{1-\frac{\Lambda(\r)}{K^2}}.
\end{equation}
\nomenclature[2n020]{$n$}{Refractive index}%
This generic expression comprises the standard definitions from neutron optics,
\begin{equation}\label{EnkK}
...
its absolute value is also much smaller than~1.
This explains why \cref{EDPsi} can be solved by a perturbation expansion.
\index{Perturbation expansion}.
