...
\E{islands}, \E{inclusions} or \E{holes}
\index{Island}%
\index{Inclusion}%
\index{Hole}%
...
namely
the \E{local monodisperse approximation} (LMA),
...
and the \E{Size-Spacing Correlation Approximation} (SSCA).
...
\Note{\indent Simulations based on \E{morphology data}
\index{Morphology!as input data}%
  % known from \IsGISAXS\
  \cite[pp.~9, 65-66]{Laz08},
...
we must first agree on relevant \E{length scales},
\index{Length scales}%
related to sample size, beam size, and coherence volume.
Given the highly anisotropic geometry of GISAS experiments,
we refer to \E{areas} rather than \E{volumes}.
By the \E{illuminated sample area}
...
By the \E{coherence area} we understand
...
and leads to the \E{diffuse scattering}
...
DA and LMA separate the contributions of the form factors and of the interference function. For disordered systems DA and LMA give the same result as the scattering vector gets larger \idest the scattered intensity is dominated by the contribution of the form factor.
