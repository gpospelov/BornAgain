...
written out without algebraic operator formalism,
...
\end{equation}
We will only need the far-field limit in $\rD$:
\begin{equation}\label{EGoFF}
  \Go_\infty(\rS,\rD) = G_\infty(\rS,\rD) + \int\!\d^3r\, \Go(\r,\rS)\delta k(\r)^2 G_\infty(\r,\rD).
\end{equation}
...
\end{equation}
Since the derivation of~\cref{EGreenFar} is invariant under an exchange of $\r$ and $\r'$,
there holds a source-detector reciprocity,\footnote
{This is a special case of a generic source-detector reciprocity theorem \cite{Pot04},
\index{Reciprocity}%
$G(\rS,\rD)=G(\rD,\rS)$, which was proven in older versions of this Manual (until 1.7.1),
but is no longer needed in the present derivation of the DWBA cross section.}
\begin{equation}
  \Go_\infty(\rS,\rD) = \Go_\infty(\rD,\rS)
\end{equation}
We multiply both sides of~\cref{EPsiPfo} with $\phi(\rD)$ to obtain
\begin{equation}\label{Epfo2}
  \Go_\infty(\rS,\rD)
  = \psi^*_\sf(\rS)\phi(\rD) + \int\!\d^3r\, \Go(\rS,\r)\delta k(\r)^2 \psi^*_\sf(\r)\phi(\rD).
\end{equation}
We compare with~\cref{EGoFF},
and read off the far-field asymptote of the distorted-waves Green function
\begin{equation}
  G_\infty(\rS,\rD) = \psi^*_\sf(\rS)\phi(\rD).
\end{equation}
