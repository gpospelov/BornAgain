...
\begin{equation}\label{EsumRF}
  \bra\psi_\si|\delta v|\psi_\sf\ket
  = \sum_j \e^{i\q_\parallel \R_{j\plll}}  \FD(\q_\parallel,\v{T}_j).
\end{equation}
The elastic scattering cross section~\cref{Exsection} can be written as
\Emph{
\begin{equation}\label{Exsection2}
  \frac{\d\sigma}{\d\Omega}
  = \sum_{jk} \e^{i\q_\plll(\R_{j\plll}-\R_{k\plll})} \FD(\q_\plll,\v{T}_j) \FD^*(\q_\plll,\v{T}_k).
\end{equation}
\vspace*{-5pt}}

In the following \Cref{SFormFactor} we discuss the form factor~$\FD$.
...
\begin{equation}\label{EFqTdef}
  F_l(\q,\v{T})
  \coloneqq \int\!\d z\, \e^{iq^u_l z} F(q_\parallel,z,\v{T}) \chi_l(z)
...
\begin{equation}\label{EqOm}
  F(\q,h,\v{\Omega},\v{P}) = \e^{iq_\parallel h} F(D_{\v{\Omega}}^{-1}\q,\v{P}),
\end{equation}
...
The case $\r=0$ (equivalent with $j=k$) needs special consideration,
which leads to the decomposition
\Emph
{\begin{equation}
  \frac{1}{N_p}\frac{\partial\sigma}{\partial\Omega}
  = I_\text{d}+I_\text{c}.
...
\begin{align*}
  & \bra \frac{d\sigma}{d\Omega}(\k_\si,\k_\sf) \ket_{\text{Off-specular}}  \\
