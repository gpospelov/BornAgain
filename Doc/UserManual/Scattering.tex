...
\begin{equation}\label{Edecompose_v}
  v(\r) \coloneqq \overline{v}(\r) + \delta v(\r).
\end{equation}
...
\begin{equation}\label{ESchrodi3}
  \left\{ \Nabla^2 + \frac{2m\omega}{\hbar} - 4\pi\overline{v}(\r) \right\}\psi(\r)
...
Accordingly, $\overline{v}(\r)$ should only contain SLD variations
...
\begin{equation}\label{Edispersion}
  K(\r)^2 = \frac{2m\omega}{\hbar} - 4\pi\overline{v}(\r).
\end{equation}
...
there is some freedom in the choice of $\overline{v}(\r)$.
In the basic variant of the Born expansion,
one makes the simplest possible choice, $\overline{v}=\text{const}$,
taking the value of $v(\r)$ outside the finite sample volume.
A constant~$\overline{v}$ implies a constant~$K$,
and so \cref{EHelmholtzHomog} reduces to the Helmholtz equation.
