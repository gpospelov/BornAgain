...
a later edition. % TODO REVERT TEMPORARY % to the later chapter~\ref{SPol}.
...
which we outline only briefly (Sec.~\ref{Swave}).
...
needed for grazing-incidence small-angle scattering (Sec.\ref{SBornApprox}).
% TODO RESTORE REF
...
have adopted the quantum-mechanical convention~(\ref{Estationarywave}),
...
and therefore we need to leave the Schrödinger equation~(\ref{ESchrodi}) intact.
Thence:
...
as explained in Sec.~\ref{Sabsorption}.}
...
In the plane-wave case (\ref{Echiq}) considered here,
