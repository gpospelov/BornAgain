...
only \E{one} species of embedded particles will be considered.

The scattering length density for an assembly of homogeneous particles of type~$\sp$
...
Per \Cref{SStep}, $\overline{v}$ determines the refractive index~$n_l$
and the wavenumber~$K_l$.
The wavefunctions $\psi_\text{i}$ and~$\psi_\text{f}$ in layer~$l$
...
\end{equation}
Thanks to the delta function in $\q_\parallel$,
the second term contributes only to the off-specular scattering,
\index{Off-specular scattering!from finite layer width}%
but not to the grazing-incidence scattering;
it is not yet implemented in BornAgain.

\Work{Until version 1.7,
BornAgain does computes neither $\Delta v_\sp$ nor $\overline{v}$
from the given $v_\sp$ and $v_\sm$.
Instead, the given $v_\sm$ is used as $\overline{v}$,
and the given $v_\sp$ as $\Delta v_\sp$.}
