...
\Note{\indent \BornAgain\ only supports discrete layers.
  Within one layer~$\il$,
  the refractive index of the matrix must be constant,
  $n_\text{m}(z)\equiv n_l$.}
%\footnote
...
over one layer
so that $\nz(z)=:\overline{n_\il^2}$ becomes $z$ independent.
...
\begin{equation}
  \chi_\text{p}(\r) := \frac{K^2}{4\pi}\left(\nz(z)-n_\text{p}^2(\r)\right)
\end{equation}
...
\begin{equation}
  ...
  \chi_\text{m}(z) &\text{~else} \end{array}\right.
\end{equation}
...
\end{equation}
We further introduce the \textit{form factor}
\index{Form factor}%
of the particle,
\begin{equation}
  F(\q,\v{T}_i) := \int\!\d^3r\,\e^{i\q\r}\chi(\r,\v{T}_i),
\end{equation}
and obtain
\begin{equation}
...
We then collect several terms into the \textit{DWBA form factor}\footnote
...
\begin{equation}
  ...
  := \sum_u C^u_l \e^{iq^u_\perp R_{i\perp}} F(\q^u,\v{T}_i).
\end{equation}
...
\end{equation}
over contributions from different layers,
\Emph{
...
\Note{\indent Simulations based on morphology data,
\index{Morphology!as input data}%
  known from \IsGISAXS\ \cite[pp.~9, 65-66]{Laz08},
  have not yet been implemented in \BornAgain,
  and are not on our agenda.
  Please contact us if you have a use case.}

Anyway, the standard approach to interpreting GISAS data
is rather in terms of statistical models for
the distribution of $\R_i$ and $\v{T}_i$.
Such models shall be introduced in the following.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Particle distribution models}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}
where the superscript indicates the dimensionality of the delta function.
The vector~$\v{T}$ comprises $d_T$ different parameters.
Some of them may only take discrete values;
for them the delta function must be read as a Kronecker delta,
and the integral in the following equation as a sum.
...
\begin{equation}\label{Epdm1}
  ...
  \int\!\d^2r\, \d^2r' \d^{d_T}\tau\, \d^{d_T}\tau'
    \PD_{ij}(\r,\r',\v\tau,\v\tau')
  \e^{i\q_{\plll}(\r-\r')}
    \FD^*(\v\tau) \FD(\v\tau'),
\end{equation}
where the integration over $\r$ is restricted to the horizontal plane.

Unless the particle coordinates are given one by one,
the formally exact distribution function~(\ref{EPDdeltas})
is useless, and needs to be replaced by a statistical model.
Before we can discuss statistical averages,
we must first agree on relevant \textit{length scales},
\index{Length scales}%
related to sample size, beam size, and coherence volume.
Given the highly anisotropic geometry of GISAS experiments,
we refer to \textit{areas} rather than \textit{volumes}.
By the \textit{illuminated sample area}
\index{Illuminated sample area}%
\index{Sample!illuminated area}%
we understand the intersection of the sample surface
with the surface illuminated by the incident beam
and the surface from which scattered radiation can reach the detector.
By the \textit{coherence area} we understand

%===============================================================================
\subsection{Homogeneous particle distributions}
%===============================================================================

In most applications, it can be assumed that the particle distribution,
averaged over the coherence volume of the scattering setup,
is constant within the illuminated volume
(if not, the Local Monodisperse Approximation might be appropriate,
see below Sect.~\ref{Slma}).
It is further standard to assume that the particle parameters $\v\tau$
all have the same distribution~$p(\v\tau)$,
which implies that there are no inter-particle correlations.
Let us denote averages under the distribution~$p(\v\tau)$ as
\begin{equation}
  \bra\FD\ket := \int\!\d^{d_T}\v\tau\,p(\v\tau)\FD(\tau).
\end{equation}
The case $i=j$ in (\ref{Epdm1}) needs special consideration,
and leads to the \textit{diffuse scattering}
...
(\textit{i.e.} the Fourier transform of the shape function - see Appendix~\ref{app:ff} for a list of form factors implemented in \BornAgain) and the composing material.
...
%===============================================================================
\subsection{Local monodisperse approximation (LMA)}
%===============================================================================
