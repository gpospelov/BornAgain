// @(#)root/qt:$Id: TGQt.cxx 42700 2012-01-18 14:33:18Z rdm $
...
Int_t TGQt::InitWindow(ULong_t window)
{
 ...
 //   QWidget *parent = (window == kDefault) ? 0 : (QWidget *)iwid(window);
   wd = new TQtWidget(parent,"virtualx",Qt::FramelessWindowHint,kFALSE);
...
//       fCodec =  QTextCodec::codecForName(fFontTextCode.toAscii()); //CP1251
       fCodec =  QTextCodec::codecForName(fFontTextCode.toStdString().c_str()); //CP1251
      if (!fCodec)
...
//         finalPixmap->save(fname,saveType.toAscii().data());
         finalPixmap->save(fname,saveType.toStdString().c_str());
      }
      delete finalPixmap;
   }
}
