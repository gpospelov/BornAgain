...
\begin{equation}\label{EsumRF}
  \bra\psi_\ti|\delta v|\psi_\tf\ket
  = \sum_j \e^{i\q_\parallel \R_{\parallel j}}  {\cal F}(\q_\parallel,\v{T_j})
\end{equation}
\vspace*{-5pt}}
In the following \Cref{SFormFactor} we discuss the form factor~${\cal F}$.
...
In \Cref{Swave21} we have applied the DWBA to a multilayer system
...
\end{equation}
$F(\q_\parallel,z,\v{T})$ is an \E{intermediate Fourier transform},
...
have been investigated in~\Cref{SLayHomo}.
...
$\v{\Omega}$ is a parameter tuple that specifies the orientation,
and $\v{P}$ collects size, shape and other parameters that determine
the refractive index profile of the particle.
This allows us to factor out the $h$~dependence of the Born form factor:
\begin{equation}\label{EFqh}
  F(\q,h,\v{\Omega},\v{P})
  = \e^{iq_\parallel h} F(\q,0,\v{\Omega},\v{P}).
\end{equation}
The parameters~$\v{\Omega}$ determine a rotation matrix~$D_{\v{\Omega}}$ so that
\begin{equation}\label{ErOm}
  F(\r,h,\v{\Omega},\v{P}) = F(D_{\v{\Omega}}\r,h,\v{0},\v{P}).
\end{equation}
Passing this through the Fourier transform, and combining it with~\cref{EFqh},
we easily obtain
\begin{equation}\label{EqOm}
  F(\q,h,\v{\Omega},\v{P}) = \e^{iq_\parallel h} F(D_{\v{\Omega}}^{-1}\q,0,\v{0},\v{P}).
\end{equation}
This reduces the implementation task to computing the Born form factor
for standard location $h=0$ and standard orientation~$\v{\Omega}=\v{0}$,
