...
//
//! @file      App/FitSuiteObserverFactory.cpp 
...
void FitSuiteObserverPrint::update(IObservable *subject)
{
    ...
    if( !fitSuite )
        throw NullPointerException("FitSuiteObserverPrint::update() -> "
                                   "Error! Can't cast FitSuite");

    if(fitSuite->isLastIteration()) {
        std::cout << std::endl;
        std::cout << "FitSuiteObserverPrint::update() -> "
            " Info. Printing results" << std::endl;
        fitSuite->printResults();
    } else {
        if( (fitSuite->getNCalls() % m_print_every_nth != 0)
            && fitSuite->getNCalls()!=0)
            return; // draw first iteration and then every n'th
        ...
        std::cout << "FitSuiteObserverPrint::update() -> Info."
                  ...
                  << fitSuite->getFitObjects()->getChiSquaredValue()
                  << std::endl;
        timeval call_time;
        gettimeofday(&call_time, 0);
        clock_t call_clock = clock();
        double timediff =
            (call_time.tv_sec - m_last_call_time.tv_sec) +
            double(call_time.tv_usec - m_last_call_time.tv_usec)/1e+06;
        std::cout << "Time spend since last call, cpu:"
                  << std::fixed << std::setprecision(2)
                  << double(call_clock-m_last_call_clock)/CLOCKS_PER_SEC
                  << " " << "sec, wall time " << timediff << "sec" << std::endl;
...
void FitSuiteObserverDraw::update(IObservable *subject)
{
    FitSuite *fitSuite = dynamic_cast<FitSuite *>(subject);
    if( !fitSuite )
        throw NullPointerException("FitSuiteObserverDraw::update() -> "
                                   "Error! Can't cast FitSuite");

    if( fitSuite->isLastIteration()) return;
    if( (fitSuite->getNCalls() % m_draw_every_nth != 0) &&
        fitSuite->getNCalls()!=0)
        return; // draw first iteration and then every n'th

    if(!m_stat_canvas) {
        //std::cout << " FitSuiteObserverDraw::update() -> "
        //             "Info. No canvas with name '" << m_canvas_name
        //          << "', creating one" << std::endl;
        std::ostringstream ostr;
        // making statistic canvas
        ostr << m_canvas_base_name << std::string("_stat");
        m_stat_canvas = new TCanvas(
            ostr.str().c_str(), ostr.str().c_str(), 800, 600);
        m_stat_canvas->Divide(2,2);
        // makind data canvas
        for(size_t i_fit_object=0; i_fit_object<
                fitSuite->getFitObjects()->size(); ++i_fit_object) {
            ostr.str(""); ostr.clear();
            ostr << m_canvas_base_name << std::string("_data_")
                 << i_fit_object << std::endl;
            TCanvas *c1 = new
                TCanvas(ostr.str().c_str(), ostr.str().c_str(), 800, 600);
            c1->Divide(2,2);
...
void FitSuiteObserverWriteTree::update(IObservable *subject)
{
    ...
    if( !top->IsOpen() ) {
        throw RuntimeErrorException("FitSuiteObserverWriteTree::update() -> "
                                    "Can't open file " + m_file_name +
                                    " for writing");
    }
