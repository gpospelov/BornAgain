...
\end{equation}
with $k_\text{i}=K$.
We can then write (\ref{Esandwich}) in more compact form as
...
\end{equation}
With (\ref{EBornQ}), the scattered flux at the detector is
\begin{equation}
  \v{J}(\r)
  = {\left|\frac{1}{r}
    \int\!{\rm d}^3r' {\rm e}^{i\v{q}\,\r'}\chi(\r')
...
\end{equation}
Using $k_\text{i}=K$ and changing notation $\r'\to\r$,
we find
...
\end{equation}
Thanks to the reciprocity theorem (\ref{Ereci}),
...
Accordingly,
the scattered far-field is still given by (\ref{EBornQ}).
