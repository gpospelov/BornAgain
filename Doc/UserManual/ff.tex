...
%The previous sections of this appendix on form factors have dealt with the Born approximation. In this case the form factor is given by a single integral over the particle shape (see equation~\ref{ffformulaBA}). But this approximation fails when multiple reflections and refractions have to be taken into account at interfaces because of the presence of a substrate or the closeness of $\alpha_i$ to the critical angle of total external reflection $\alpha_c$. The first order correction to the scattering theory is the Distorted Wave Born Approximation (DWBA), whereas the Born approximation is the zeroth order.  In the followings, the expression of the form factor in the DWBA is going to be given for different layouts of particles: particles deposited on a substrate and buried particles.

%\paragraph{Particles deposited on a substrate}
%In this configuration, the particles are sitting on top of a substrate layer, in the air. In the DWBA the expression of a form factor becomes 
%\begin{align*}
%F_{\rm{DWBA}}(q_{\parallel}, k_{i,z}, k_{f,z}) &= F_{\rm{BA}}(q_{\parallel}, -k_{f,z}-k_{i,z})+ R(\alpha_i) F_{\rm{BA}}(q_{\parallel}, k_{i,z}-k_{f,z}) \\
%&+ R(\alpha_f) F_{\rm{BA}}(q_{\parallel}, -k_{i,z}+k_{f,z}) + R(\alpha_i) R(\alpha_f) F_{\rm{BA}}(q_{\parallel},k_{i,z}+k_{f,z}), 
%\end{align*}
%where $q_{\parallel}$ is the component of the scattering beam in the plane of the interface, $k_{i,z}$ and $k_{f,z}$ are the z-component of the incident and scattered beam, respectively. Let's remember that the $z$ axis is pointing upwards. $R(\alpha_i)$, $R(\alpha_f)$ are the reflection coefficients in incidence and reflection. They are defined as $R=\dfrac{k_z+\sqrt{n_s^2k_0^2-|k_{\parallel}|^2}}{k_z-\sqrt{n_s^2 k_0^2-|k_{\parallel}|^2}}$, where $n_s$ is the subsrate refractive index, $k_0$ is the wavelength in vacuum ($2\pi /\lambda$). % (see \cite{Yok07} for details). In the DWBA, the form factor depends on 

%Figure~\ref{fig:SchemDWBA} illustrates the four scattering processes for a supported particle, taken into account in the DWBA. The first term corresponds to the Born approximation. Each term is weighted by a Fresnel coefficient. 

%\begin{figure}[h]
%\begin{center}
%\includegraphics[width=\textwidth]{Figures/drawingDWBA}
%\end{center}
%\caption{Schematic views of the different terms appearing in the expression of the form factor under DWBA.}
%\label{fig:SchemDWBA}
%\end{figure}

%%The four terms involved are associated to different scattering events, which involve or not a reflection of either the incident beam or the final beam collected on the detector. These waves interfere coherently, giving rise to the effective form factor (FDWBA), in which the classical form factor comes into play but with respect to the specific wavevector transfers. Each term is weighted by the corresponding Fresnel reflection coefficient R, either in incidence or in reflection. The reduced reflectivity of the substrate by an uncorrelated roughness can be calculated in a classical way with a mean standard deviation. The scattered cross section is proportional to the scattered intensity and thus to the modulus square of the Fourier transform of the electron density. 
%From Lazzari 2002, DWBA= application of first order perturbation induced by island's roughness to the correct unperturbed wave: incident - reflected - refracted waves at the substrate surface.


%Script~\ref{lst:badwba} illustrates the difference between BA and DWBA for the simple case of:
%\begin{itemize}
...
%\item in the BA, the sample is composed of the particles in air.
%\end{itemize} 

%Figure~\ref{fig:spheroidbadwba} shows the intensity contourplot
%generated using this script with spheroids.

%\begin{lstlisting}[language=python, style=eclipseboxed,numbers=none,nolol,caption={\Code{Python} script to generate figures~\ref{fig:spheroidbadwba}. The difference between BA and DWBA in this simple case is the absence or presence of a substrate layer in the sample.},label={lst:badwba}]
%import numpy
%import matplotlib
%import pylab
...
%\begin{figure}[ht]
%\hfill
%\subfigure[Born Approximation]{\includegraphics[width=5cm]{Figures/ffspheroidBA}}
%\hfill
%\subfigure[DWB Approximation]{\includegraphics[width=5cm]{Figures/ffspheroidDWBA}}
%\hfill
...
%\ImportantPoint{Remark:}{ An example of listings for BA and DBWA using cylindrical particles can be found in the Examples folder of \BornAgain\ source code : \Code{CylinderFormFactorDWBA.py}, \Code{CylinderFormFactorBA.py}.}

%\paragraph{Buried particles} 
%to be completed
\newpage{\cleardoublepage}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hfill
\caption{Example of a core-shell particle composed of a parallelepiped with a pyramidal  inset. The relative core shell position is marked by the position of its center of gravity (blue point $\color{blue}\bullet$) with respect to the center of gravity of the parallelepiped (red point $\color{red}\bullet$). }
\label{fig:coreshell}
\end{figure}

\begin{lstlisting}[language=python, style=eclipseboxed,numbers=none,nolol,caption={\Code{Python} script to create a core-shell particle made of a parallelepiped with a pyramidal shifted inset.},label={lst:cshellsample}]
def get_sample():
    """
    ...
    # collection of particles
    outer_ff = FormFactorParallelepiped(16.0*nanometer, 8.0*nanometer) 
...
\end{center}
\caption{Intensity map of a core-shell form factor in Born Approximation using  \Code{FormFactorParallelepiped(16*nanometer, 8*nanometer)} and \Code{FormFactorPyramid(12*nanometer, 7*nanometer, 60*degree)} for the outer and inner shells, respectively. The core particle is shifted by 1.5~nm in the $x$-direction with respect to the centre of the outer shell. The sample used to generate this figure is listed in~\ref{lst:cshellsample}.  There is no substrate and no interference between the particles.}
