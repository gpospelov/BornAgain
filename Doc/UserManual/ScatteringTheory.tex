...
\E{lost} by the scattered neutron,
and \E{gained} by the sample.}
...
is limited to the \E{illuminated sample area}
...
We shall also discuss how to compare measured data with simulations,
and how to present data on a physically meaningful scale.
...
\index{Pixel|see {Detector pixel}}%
...
using the horizontal declination angle~$\phi_\tf$
and the vertical glancing angle $\alpha_\tf$.
The stereographic projection of $(\alpha_\tf,\phi_\tf)$ into
\index{Stereographic projection}%
the detector plane~$(y,z)$ is known as the \E{gnomic projection}.
\index{Gnomic projection}%
...
\index{Detector pixel!solid angle}%
is seen from the sample is in linear approximation
\begin{equation}
  \Delta\Omega
...
\begin{equation}\label{Ekifdet}
  \k_\tf=\frac{K}{L^2+y^2+z^2}\left(\begin{array}{c}
   L\\
   y\\
   z\end{array}\right).
\end{equation}
...
%===============================================================================
\subsection{Data representation without ``data reduction''}
%===============================================================================
