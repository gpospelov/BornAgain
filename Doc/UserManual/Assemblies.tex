...
and by a parameter vector~$\v\tau_i$ that collects
...
\begin{equation}
  \chi_\text{p}(\r) = \sum_i \chi(\r-\R_i,\v\tau_i).
\end{equation}
We further introduce the \textit{form factor}
\index{Form factor}%
of the particle,
\begin{equation}
  F(\q,\v\tau_i) := \int\!\d^3r\,\e^{i\q\r}\chi(\r,\v\tau_i),
\end{equation}
and obtain
\begin{equation}
  \chi_\text{p}(\q) = \sum_i \e^{i\q\R_i} F(\q,\v\tau_i).
\end{equation}
...
\begin{equation}
  \FD_l(\k_\ti,\k_\tf;\v\tau_i)
  := \sum_u C^u_l \e^{iq^u_\perp R_{i\perp}} F(\q^u,\v\tau_i).
\end{equation}
...
With this restriction, (\ref{Eass4}) becomes an incoherent
sum over contributions from different layers,
\Emph{
\begin{equation}\label{Eass3}
  \frac{\partial\sigma}{\partial\Omega}
  = \sum_{l} \sum_{ij} \e^{i\q_{l\plll}(\R_{i\plll}-\R_{j\plll})}
    \FD^*_l(\v\tau_i) \FD_m(\v\tau_j).
\end{equation}
\vspace*{-5pt}}
...
for a given set of particle coordinates $\{\R_i,\v\tau_i\}$.

\Note{Simulations based on morphology data,
  known from \IsGISAXS\ \cite[pp.~9, 65-66]{Laz08},
  have not yet been reimplemented in \BornAgain,
