...
\index{Pauli vector}%
\index{Pauli matrix}%
The magnetic coupling can cause Larmor precession of the neutron spin,
...
\begin{align}
  \Lambda(\r) &\coloneqq \mv(\r)  &\text{ neutron (scalar),}\\
  \TENS{\Lambda}(\r) &\coloneqq \mv(\r)+\overline{\v{h}}(\r)\v\sigma &\text{ neutron (spinorial),}\\
  \Lambda(\r) &\coloneqq K^2(\epsilon(\r)-1) &\text{ X-rays.}
\end{align}
...
with
\begin{align}
  U(\r) &\coloneqq \delta v(\r)  &\text{ neutron (scalar),}\\
  \TENS{U}(\r) &\coloneqq \delta v(\r)+\delta{\v{h}}(\r)\v\sigma &\text{ neutron (spinorial),}\\
  \TENS{U}(\r) &\coloneqq K^2\delta \epsilon(\r) &\text{ X-rays.}
\end{align}
\Cref{EDPsi} is the base for computing scattering cross sections in
distorted-wave Born approximation (DWBA).
\index{Distorted-wave Born approximation}%
The DWBA generalizes the standard Born approximation,
\index{Born approximation}%
which corresponds to the special case~$\v\Lambda(\r)=0$.

%===============================================================================
\subsection{Refractive index}\label{Srefridx}
%===============================================================================

\index{Refractive index|(}%
\index{Index of refraction|see {Refractive index}}%

We pause here to mention an alternative parametrization of scattering length densities
or dielectric permittivities, namely the refractive index.
In neutron optics,
it is customary to express $\mv(\r)$ by the \E{refractive index}
\begin{equation}\label{EnkK}
  n(\r)\coloneqq\sqrt{1-\frac{4\pi\mv(\r)}{K^2}}.
\end{equation}
\nomenclature[2n020]{$n$}{Refractive index}%
For electromagnetic waves,
the refractive index is simply
\begin{equation}
  n(\r)\coloneqq\sqrt{\epsilon(\r)}.
\end{equation}
...
\end{equation}
So we obtain the wave equation
\begin{equation}\label{ENabNabE}
  \left\{\Nabla^2-\Nabla\cdot\Nabla+ K^2-4\pi v(\r)\right\}\v{E}(\r)=0.
\end{equation}
