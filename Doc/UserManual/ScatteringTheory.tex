...
\begin{equation}\label{Escabouco}
  \psi(\r)
  \to \psi_\ti(\r) + f(\vartheta,\varphi)\frac{{\rm e}^{iKr}}{r}
  \text{~for~}r\to\infty,
\end{equation}
...
\begin{equation}\label{EBornS}
  \psi_\text{s}(\r)
  :=
  \int\!{\rm d}^3r'\, G(\r,\r')\frac{\chi(\r')}{4\pi}\psi_\ti(\r').
\end{equation}
...
\begin{equation}\label{Etrama}
  \bra \psi_\tf|\chi|\psi_\ti\ket
  := \int\!{\rm d}^3r\, \psi^*_\tf(\r)\chi(\r)\psi_\ti(\r).
\end{equation}
...
\psi_\ti(\r) = {\rm e}^{i \k_\parallel\r_\parallel} \phi(z).
\end{equation}
...
\begin{equation}\label{EBFar}
  B_\text{far}(\r,\rD)
  =\frac{{\rm e}^{iKr_\text{D}}}{4\pi r_\text{D}}{\rm e}^{-i\k_\tf \r}.
\end{equation}
