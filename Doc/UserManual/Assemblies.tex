...
To apply the DWBA to this scattering problem,
we must choose a decomposition~\cref{Edecompose_vz} of~$v(\r)$.
BornAgain only supports step profiles for~$v_\sm(z)$.
Within one layer~$l$, the matrix has a constant SLD~$v_{\sm l}$.
\Work{Currently (at least up to version 1.7),
BornAgain puts $\overline{v}_l = v_{\sm l}$.
In the future, this may be replaced by a better approximation
that takes the average contribution from the particle SLD into account.}
\iffalse
For each layer
and $\chi_\sm(\r)\coloneqq 1-\chi_\sp(\r)$.
The average SLD in layer~$l$ is then
\begin{equation}
...
The fluctuating term in the decomposition~\cref{Edecompose_vz} is
\begin{equation}
  \delta v_l(\r) = v_{\sm l} + \Delta v_\sp(\r) - \overline{v}_l.
\end{equation}
With $\overline{v}_l = v_{\sm l}$, this reduces to
...
with the indicator function
\begin{equation}\label{EdefChiP}
  \chi_l(z)\coloneqq\left\{\begin{array}{ll}
  1&\text{~if $z$ in Layer~$l$,}\\[.2ex]
  0&\text{~otherwise.} \end{array}\right.
\end{equation}
