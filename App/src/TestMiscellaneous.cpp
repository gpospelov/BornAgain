...
void TestMiscellaneous::test_DoubleToComplexInterpolatingFunction()
{
    MultiLayer *sample = dynamic_cast<MultiLayer *>(
            SampleFactory::createSample("MultilayerOffspecTestcase1a"));
    ...
    std::map<double, SpecularMatrix::MultiLayerCoeff_t> RT_coeff_map;
    for (size_t i=0; i<p_alpha_axis->getSize(); ++i) {
        ...
        MatrixCalculator.execute(*sample, kvec, coeffs);
        RT_coeff_map[angle] = coeffs;
    }
    ...
    for(size_t i_layer=0; i_layer<sample->getNumberOfLayers(); ++i_layer) {
        ...
        for (std::map<double, SpecularMatrix::MultiLayerCoeff_t>::const_iterator
                it=RT_coeff_map.begin(); it!=RT_coeff_map.end(); ++it)
        {
        ...
        DoubleToComplexInterpolatingFunction R_function(R_map,
                DoubleToComplexInterpolatingFunction::Nearest);
    ...
    for(int i_point=0; i_point < npoints; i_point++){
        int i_layer_sel = 0;
        double angle = alpha_min + i_point*(alpha_max-alpha_min)
                       / double(npoints-1);
        kvector_t kvec;
        kvec.setLambdaAlphaPhi(1.4*Units::angstrom, angle, 0.0);
        SpecularMatrix::MultiLayerCoeff_t coeffs;
        ...
        std::cout << i_point << " " << angle << " true R:"
                << coeffs[i_layer_sel].R() << " interp:" << R << " "
                << std::abs(R - coeffs[i_layer_sel].R()) << std::endl;
        gr1_exact->SetPoint(i_point, angle, std::abs(coeffs[i_layer_sel].R()) );
        gr2_interp->SetPoint(i_point, angle, std::abs(R) );
        gr3_diff->SetPoint(i_point, angle, std::abs(R)
                           - std::abs(coeffs[i_layer_sel].R()) );
    }
