...
\begin{equation}
  \delta v(\r) = \Delta v_\sp(\chi_p(\r)-\rho_\sp)
\end{equation}
with $\Delta v_\sp\coloneqq v_\sp-v_\sm$.

Since our material layer has a constant~$\overline{v}$,
the wavefunctions~$\psi_\text{i,f}^\pm$ are plane waves,
and the DWBA matrix elements of~\cref{Edwba} are all
Fourier transforms~\cref{Echij}
of $\chi_l(z)\delta v(\r)$.
we consider a layer with a $z$-independent
