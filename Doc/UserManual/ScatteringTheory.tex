...
$\Psi_{\rm i}$ that must be present in any scattering experiment.
...
\begin{equation}\label{EBornS}
  \Psi_{\rm s}(\r)
  \doteq 
  \int\!{\rm d}^3r' G(\r,\r')\frac{\chi(\r')}{4\pi}\Psi_{\rm i}(\r').
\end{equation}
...
\begin{equation}\label{EHelmholtzGradedHomog}
  \left\{\Nabla^2+K^2\nz^2(z)\right\}\Psi_{\rm i}(\r) = 0,
\end{equation}
which suggests the factorization ansatz
\begin{equation}\label{Ewave3}
\psi_{\rm i}(\r) = {\rm e}^{i \k_\parallel\r_\parallel} \phi(z).
\end{equation}
...
\end{equation}
On the surface $\partial\Sphere$, which lies outside $\mathcal{M}$,
...
the differential cross section (\ref{Ecross1}) becomes
\begin{equation}\label{Ecross2}
  ...
  = {\left|
    ...
    \right|}^2.
\end{equation}
If the average refractive index $\nz(z)$ is constant within a layer~$j$,
then the contribution of this layer to the scattered intensity
can be written as
