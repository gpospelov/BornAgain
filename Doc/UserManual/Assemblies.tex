...
\Note{\indent In \BornAgain, there is no explicit support for particles attached
  to a substrate. Instead, users must construct a layer on top of the
  substrate that consists of vacuum (or air) plus the embedded particles.
  Similarly, an extra layer must be created to describe particles buried in
  the substrate.}
...
\Note{\indent Presently,
  in \BornAgain\ particles must be confined to given layers,
  and within one layer~$j$, a constant $\nz(z)=n_j^2$ is used.}

Once $\nz(z)$ is computed in whatever approximation,
the perturbation potential is given by (\ref{EChiGraded}),
which takes the form
\begin{equation}
  \chi_p(\r) := \frac{K^2}{4\pi}\left(\nz(z)-n_p^2(\r)\right)
\end{equation}
for the particles, and similarly $\chi_0(z)$ for the matrix.
The overall potential
\begin{equation}
  \chi(\r)=\left\{\begin{array}{ll}
  \chi_p(\r)&\text{~if $\r$ in particle,}\\[.2ex]
  \chi_0(z) &\text{~else} \end{array}\right.
\end{equation}
\nomenclature[1χ032 0]{$\chi_0(z)$}{Perturbative potential of embedding material (``matrix'')}%
\nomenclature[1χ032 2p010]{$\chi_p(\r)$}{Perturbative potential of embedded particles of type~$p$}%
can be rearranged as
\begin{equation}\label{Echi0p}
  \chi(\r)=\chi_0(z) + \left\{\begin{array}{ll}
  \tilde\chi_p(\r)&\text{~if $\r$ in particle,}\\[.2ex]
...
\begin{equation}
  \tilde\chi_p(\r)
  := \chi_p(\r)-\chi_0(z)
  = \frac{K^2}{4\pi}\left(n_0^2(z)-n_p^2(\r)\right),
\end{equation}
\nomenclature[1χ033 2p010]{$\tilde\chi_p(\r)$}{Renormalized perturbative potential of embedded particles of type~$p$}%
which is 0 unless $\r$ lies within a particle.
...
We specialize to the case of a layer~$j$ with constant~$\nz(z)=n_j^2$
so that the $\psi^\pm_{wj}$ are plane waves.
...
\end{equation}
\nomenclature[2s000 2i000 2n000 2c000 0]{sinc}{Sinus cardinalis: $\text{sinc}(x)=\sin(x)/x$}%
For the second term,
we abbreviate
\begin{equation}
