...
The solution avalaible from the literature~\cite{LeMi83}
...
To derive the form factor of a polyhedron,
we start from the divergence theorem
\begin{equation}
  \iiint\!\d^3r\,\Nabla\v{H} = \iint\!\d^2r\, \n\v{H}.
\end{equation}
With the choice $\v{H}\coloneqq\q^*(\e^{i\q\r}-1)$,
we obtain
\begin{equation}
  F(\q) = \frac{\q^*}{iq^2} \iint\!\d^2r\, \n \left(\e^{i\q\r}-1\right).
\end{equation}
For a polygon, the surface integral can be written
as a sum over integrals over planar faces,
\begin{equation}
  F(\q) = \frac{\q^*}{iq^2} \sum_k \n_k \iint\!\d^2r\, \left(\e^{i\q\r}-1\right).
\end{equation}
To proceed, we use Stokes' theorem~(\ref{EStokes})
with the choices
\begin{equation}
  \begin{array}{@{}lcl}
\qp&\coloneqq& \q_{\parallel k}\coloneqq\q-(\q\n_k)\n_k,\\
\v{a}&\coloneqq&\n\times\uqp^*,\\
\v{G}&\coloneqq&\v{a}\e^{i\qp\r}-i(\qp\times\v{a})\times\r/2.
  \end{array}
\end{equation}
We obtain
\begin{equation}
  \iint\!\d^2r\, \left(\e^{i\q\r}-1\right)
