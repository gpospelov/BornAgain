...
void PyCoreTest::runTest()
{
    ...
    std::string output_filename = m_output_filename+".int";
...
//         << "    os.path.realpath(__file__))[0],'..', '..', '..', '..', 'lib')))\n\n"
    ...
    std::cout << "Core simulation will be compared with " << output_filename <<
    ...
    const std::unique_ptr<OutputData<double> > P_domain_data(
        IntensityDataIOFactory::readOutputData(output_filename));
