...
\index{Perturbation potential!neutron}%
\index{Potential|see{Perturbation potential}}%
...
For the time being,
the right-hand side of~\cref{ESchrodiK} will be treated as a given inhomogeneity.
On this premise, equation~\cref{ESchrodiK} shall be solved by the Green function method.
The homogeneous wave equation
\begin{equation}\label{EHomoK}
  \left\{\Nabla^2+k(\r)^2\right\}\psi(\r) = 0
\end{equation}
correspondes to the zeroth order of the Born approximation,
and will yield the incident wave~$\psi_\si(\r)$.
The Green function~$G(\r,\r')$ obyes the wave equation with an isolated inhomogeneity,
\begin{equation}\label{EGreenK}
  \left\{\Nabla^2+k(\r)^2\right\}G(\r,\r') = \delta(\r-\r').
\end{equation}
These two equations are addressed in full generality by the distorted-wave Born approximation
(\cref{SDWBA}).
\index{Distorted-wave Born approximation}%
Here, we will substantially simplify them.
...
Accordingly, we have a constant~$k(\r)=K$.
...
\end{equation}
with $k_\si=K$.
Superpositions of plane waves shall be discussed
when we come to instrumental resolution effects (\cref{SBeam}).
...
With the incident plane wave~\cref{EPsi0Plane},
...
%===============================================================================
\subsection{OLD MAT}\label{SDWBA1}
%===============================================================================
...
\index{Perturbation potential!X-ray}%
\begin{equation}
